...
from sqlalchemy import Column, Integer, String, Boolean, delete, insert, update
...
class Adapter(AsyncAdapter):
    ...
    def _softdelete_query(self, stmt):
        """Apply soft delete filter to exclude soft-deleted records."""
    ...
    async def remove_policy(self, sec, ptype, rule):
        ...
        async with self._session_scope() as session:
            if self.softdelete_attribute is None:
                ...
                for i, v in enumerate(rule):
                    update_stmt = update_stmt.where(getattr(self._db_class, "v{}".format(i)) == v)
                update_stmt = update_stmt.values({self.softdelete_attribute.name: True})
    ...
    async def remove_policies(self, sec, ptype, rules):
        ...
        async with self._session_scope() as session:
            if self.softdelete_attribute is None:
                ...
                stmt = stmt.where(self._db_class.ptype == ptype)
                rules_zip = zip(*rules)
                for i, rule in enumerate(rules_zip):
                    stmt = stmt.where(or_(getattr(self._db_class, "v{}".format(i)) == v for v in rule))
                
                update_stmt = update(self._db_class).where(self._db_class.ptype == ptype)
                rules_zip = zip(*rules)
                for i, rule in enumerate(rules_zip):
                    update_stmt = update_stmt.where(or_(getattr(self._db_class, "v{}".format(i)) == v for v in rule))
                update_stmt = update_stmt.values({self.softdelete_attribute.name: True})
        ...
        async with self._session_scope() as session:
            ...
            if self.softdelete_attribute is None:
                ...
                for i, v in enumerate(field_values):
                    if v != "":
                        v_value = getattr(self._db_class, "v{}".format(field_index + i))
                        update_stmt = update_stmt.where(v_value == v)
                update_stmt = update_stmt.values({self.softdelete_attribute.name: True})
                r = await session.execute(update_stmt)
    ...
    async def _update_filtered_policies(self, new_rules, filter) -> List[List[str]]:
        ...
        async with self._session_scope() as session:
            ...
            stmt = select(self._db_class)
            stmt = self._softdelete_query(stmt)
            stmt = stmt.where(self._db_class.ptype == filter.ptype)
